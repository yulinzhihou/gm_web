...
class SyncGem extends AbstractTool
{
    /**
    ...
    public function handle()
    {
        ...
        $equips = file_get_contents(public_path()."/config_file/GemInfo.txt");
        $header = [
        ...
        foreach($equip as $value){
            ...
            Redis::hmset('gem',$newCols);
            $ret = Gem::updateOrCreate(['id'=>$cols[0]],$allData);
            $result = $result && $ret;
        }
        Redis::hmset('gem_id_name',['id_names'=>json_encode($idNames),'ids'=>json_encode($ids)]);
        if ($result) {
